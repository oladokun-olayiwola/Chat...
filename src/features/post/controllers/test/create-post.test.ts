...
import * as postServer from '@socket/posts';
  ...
  describe('postWithImage', () => {
    it('should throw an error if image is not available', async () => {
      ...
      jest
        ...
        .mockImplementation((): any => Promise.resolve({ version: '', public_id: '', message: 'Missing required parameter - file' }));

      await Create.prototype.PostwithImage(req, res).catch((error: CustomError) => {
        expect(error.statusCode).toEqual(400);
        expect(error.serializeErrors().message).toEqual('Missing required parameter - file');
      });
    });
